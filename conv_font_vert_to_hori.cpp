...
int main (void) {
    ...
    for (int iz = 0; iz < 256; iz++)
    {
        ...
        unsigned char charMade[8] = {0,0,0,0,0,0,0,0};

        for (int ix = 0; ix < 8; ix++)
        {
            unsigned char pLido = cp437_font[iz][ix];

            for (int iy = 0; iy < 8; iy++)
                charMade[iy] |= (((pLido >> iy) & 0x01) << (7 - ix));
        }    

        for (int iy = 0; iy < 8; iy++)
        {
            printf("$%02X", charMade[iy]);
