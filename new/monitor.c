...
unsigned char inputLine(unsigned int pQtdInput, unsigned char pTipo)
{
    ...
    {
        // Apresenta a linha na tela, e posiciona o cursor na tela na primeira posicao valida (depois do numero da linha)
        iw = strlen(vbuf) / 40;

        printText(vbuf);

        iey = *videoCursorPosRowY;
        iw++;
    ...
    while (1)
    {
        ...
        *vBufReceived = 0x00;
        ...
        if (vtec)
        {
            ...
            /*else if (vtec == 0x11)    // UpArrow (17)
            {
                // TBD
            }
            else if (vtec == 0x13)    // DownArrow (19)
            {
                // TBD
            }*/
            else if (vtec == 0x12)    // LeftArrow (18)
            {
                if (iPos > 0)
                {
                    printChar(vbuf[iPos],0);
                    ...
                    if (*videoCursorPosColX == 0)
                        *videoCursorPosColX = 256;
                    else
                        *videoCursorPosColX--;
                    vdp_set_cursor(*videoCursorPosColX, *videoCursorPosRowY);
                }
            }
            else if (vtec == 0x14)    // RightArrow (20)
            {
                if (iPos < strlen(vbuf))
                {
                    printChar(vbuf[iPos],0);
