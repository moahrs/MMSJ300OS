...
*                        - Controle de Interrupcoes VDP e PS/2
...
#define versionBios "0.4a"
#define versionBasic "0.0.0"
...
unsigned char inputLine(unsigned int pQtdInput, unsigned char pTipo)
{
    ...
    while (1)
    {
        ...
        *vBufReceived = 0x00;
        ...
        // Se nao for string ($), só aceita numeros
        if (pTipo!='$'&&vtec!='.'&&vtec>0x1F&&(vtec<0x30 || vtec>0x39))
            vtec = 0;

        // Só aceita ponto de for numero real (#) ou string ($)
        if (vtec=='.'&&pTipo!='#'&&pTipo!='$')
            vtec = 0;
...
int processCmd(void)
{
    ...
    unsigned char sqtdtam[10], cuntam, vparam[32], vparam2[16], vparam3[16], vpicret;
    int vRet = 1;
    ...
    if (linhacomando[0] != 0)
    {
        ...
        {
            printText("Unknown Command or File !!!\r\n\0");
        }
    }
    ...
    *(vmfp + Reg_IERA) &= 0x3E;
    ...
    while(1)
    {
        ...
        while(!(*(vmfp + Reg_RSR) & 0x80))
        {
            ...
            vTimeout++;
            if (vTimeout > 30000000) // +/- 5 min
                break;
        };

        if (vTimeout > 30000000)
            break;
